...
class Battery:
    ...
    def judge(self, action):
        new_E = self.E - Actions[action] * self.Pmax * 0.5
        flag = bool(new_E<0 or new_E>self.capacity)
        if not flag:
            return flag
        else:
            k = []
            for i in range(4):
                new_E = self.E - Actions[i] * self.Pmax
                if not bool(new_E<0 or new_E>self.capacity):
                    k.append(i)
            return k

    def charge(self, action):
        self.E -= Actions[action] * self.Pmax * 0.5
...
class MicroGrid(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    **Note**: k+1时刻的电池电量E取决于k时刻的电池电量和采取的动作
    ...
    def step(self, action):
        ...
        flag = self.battery.judge(action)
        if flag == False:
        ...
        if not terminated:
            reward = (Wind + pv + Actions[action] * self.BatPmax * 0.5 - load) * price#千刀
        else:
            reward = 0
